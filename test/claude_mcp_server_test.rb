...
class ClaudeMcpServerTest < Minitest::Test
  ...
  def test_logging_setup_creates_directory
    ...
    logs_dir = File.join(@tmpdir, ".claude-swarm", "logs")

    assert Dir.exist?(logs_dir)
  end
  ...
  def test_logging_with_environment_timestamp
    ...
    log_file = File.join(@tmpdir, ".claude-swarm", "logs", "session_20240101_120000.log")
  ...
  def test_logging_without_environment_timestamp
    ...
    log_file = File.join(@tmpdir, ".claude-swarm", "logs", "session_#{timestamp}.log")
  ...
  def test_task_tool_logging
    ...
    # Check log file
    log_file = Dir.glob(File.join(@tmpdir, ".claude-swarm", "logs", "session_*.log")).first
    log_content = File.read(log_file)
